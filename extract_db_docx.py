...
possible_paths = [
    r"\\Mac\Home\Documents\1_monhoc\KhoaLuan\cơ sở dữ liệu dự án.docx",
    r"C:\Users\hochidung\Documents\1_monhoc\KhoaLuan\cơ sở dữ liệu dự án.docx",
    r"\\Mac\Home\Documents\1_monhoc\KhoaLuan\*.docx",
    r"C:\Users\hochidung\Documents\1_monhoc\KhoaLuan\*.docx",
    r"\\Mac\Home\Documents\*.docx",
    r"C:\Users\hochidung\Documents\*.docx"
]
...
for p in possible_paths:
    matches = glob.glob(p)
    for m in matches:
        if "cơ sở dữ liệu" in m.lower() or "du an" in m.lower() or "co so du lieu" in m.lower() or "KhoaLuan" in m:
            target_file = m
            break
    if target_file:
        break
...
if not target_file:
    # Try searching recursively in Documents
    for root_dir in [r"\\Mac\Home\Documents", r"C:\Users\hochidung\Documents"]:
        for root, dirs, files in os.walk(root_dir):
            for f in files:
                if f.endswith('.docx') and ('cơ sở dữ liệu' in f.lower() or 'du an' in f.lower() or 'khoaluan' in root.lower()):
...
out_path = r'C:\laragon\www\KhoaLuanCuNhan\db_docx_extracted.txt'
