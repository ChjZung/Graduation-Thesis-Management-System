...
@section('content')
    ...
    <div class="card card-premium">
        <div class="card-header-premium d-flex justify-content-between align-items-center">
            <span><i class="fa-solid fa-book-open text-primary me-2"></i> Quản Lý Ngành Học</span>
            <div class="d-flex gap-2">
                <a href="{{ route('admin.import.template', 'nganh') }}"
                ...
                <a href="{{ route('nganh.create') }}" class="btn btn-success btn-sm rounded-pill px-3 fw-bold">
                ...
                <table class="table table-custom table-hover mb-0 align-middle">
                    ...
                    <tbody>
                        @forelse($nganhs as $nganh)
                            <tr>
                                <td><span class="badge bg-light text-dark fw-bold border">{{ $nganh->MaNganh }}</span></td>
                                <td class="fw-bold text-primary-custom">{{ $nganh->TenNganh }}</td>
                                <td>
                                ...
                                <td class="text-center">
                                    <a href="{{ route('nganh.edit', $nganh->MaNganh) }}"
                                        class="btn btn-sm btn-light text-primary me-1 rounded-circle" title="Sửa">
                                        <i class="fa-solid fa-pen"></i>
                                    </a>
                                    <form action="{{ route('nganh.destroy', $nganh->MaNganh) }}" method="POST" class="d-inline"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="alert alert-light border small text-muted mb-0">
                            <i class="fa-solid fa-circle-info me-1"></i> Tải <a
                                href="{{ route('admin.import.template', 'nganh') }}" class="fw-bold">File mẫu .xlsx</a> để
