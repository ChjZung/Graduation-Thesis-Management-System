...
<div class="card card-premium">
    <div class="card-header-premium">
        <span><i class="fa-solid fa-user-check me-2"></i> Duyệt Đơn Đăng Ký Đề Tài Của Nhóm Sinh Viên</span>
        ...
        <div class="table-responsive">
            <table class="table table-custom table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <div class="fw-bold text-primary">{{ $dk->nhom->TenNhom ?? 'Chưa rõ' }}</div>
                            <div class="small text-muted"><i class="fa-solid fa-user-tag me-1"></i>Trưởng nhóm: {{ $dk->nhom->truongNhom->HoTen ?? '' }}</div>
                        </td>
                        <td>
                            <div class="fw-bold text-dark">{{ $dk->deTai->TenDeTai ?? '' }}</div>
                            <div class="small text-muted"><span class="badge-code">Mã ĐT: {{ $dk->MaDeTai }}</span></div>
                        </td>
                        <td>
                            <div class="fw-bold text-dark">{{ $dk->deTai->giangVien->HoTen ?? 'Chưa rõ' }}</div>
                        ...
                        <td class="text-center">
                            ...
                            <div class="d-flex justify-content-center gap-1">
                                <form action="{{ route('admin.duyet_dangky.approve', $dk->MaDangKy) }}" method="POST" class="d-inline">
                                    @csrf
                                    <button type="submit" class="btn btn-sm btn-success rounded-pill px-3" onclick="return confirm('Xác nhận duyệt đề tài này cho nhóm SV?');">
                    ...
                    <tr>
                        <td colspan="5">
                            <div class="empty-state-box">
                                ...
                                <h6>Không có đơn đăng ký đề tài nào cần xử lý</h6>
                                <p class="small text-muted mb-0">Tất cả nguyện vọng đăng ký của nhóm sinh viên đã được phê duyệt.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
