...
@section('page_title', 'Chỉnh Sửa Bộ Môn Chuyên Môn')
        ...
        <div class="card card-create-huit">
            <div class="card-header-huit">
                <div class="card-title-huit">
                    <i class="fa-solid fa-pen-to-square text-white me-1"></i> Chỉnh Sửa Thông Tin Bộ Môn: {{ $bomon->TenBoMon }}
                </div>
            </div>
            <div class="card-body p-4 p-md-5">
                ...
                <form method="POST" action="{{ route('bomon.update', $bomon->MaBoMon) }}">
                    @csrf
                    @method('PUT')
                    
                    <div class="row g-4 mb-4">
                        <div class="col-md-5">
                            <label class="form-label-huit">Mã Bộ Môn</label>
                            <input type="text" class="form-control form-control-huit bg-light" value="{{ $bomon->MaBoMon }}" readonly>
                        </div>
                        <div class="col-md-7">
                            <label class="form-label-huit">Tên Bộ Môn Chuyên Môn <span class="text-danger">*</span></label>
                            <input type="text" name="TenBoMon" class="form-control form-control-huit" value="{{ old('TenBoMon', $bomon->TenBoMon) }}" required>
                        </div>
                    </div>
                    ...
                    <div class="row g-4 mb-4">
                        <div class="col-md-7">
                            ...
                            <select name="MaKhoa" class="form-select form-select-huit" required>
                                ...
                                @foreach($khoas as $k)
                                    <option value="{{ $k->MaKhoa }}" {{ old('MaKhoa', $bomon->MaKhoa) == $k->MaKhoa ? 'selected' : '' }}>
                                        {{ $k->TenKhoa }} ({{ $k->MaKhoa }})
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="TruongBoMon" class="form-select form-select-huit">
                                ...
                                @if(isset($giangViens))
                                    @foreach($giangViens as $gv)
                                        ...
                                        <option value="{{ $val }}" {{ old('TruongBoMon', $bomon->TruongBoMon ?? '') == $val ? 'selected' : '' }}>
                                            {{ $val }}
                                        </option>
                                    @endforeach
                                @endif
                    ...
                    <div class="card-footer-huit mx-n4 mb-n4 mt-5">
                        <button type="submit" class="btn btn-save-huit">
                            <i class="fa-solid fa-check me-1"></i> Cập Nhật Bộ Môn
                        </button>
