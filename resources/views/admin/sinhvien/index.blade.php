...
<div class="card card-premium">
    <div class="card-header-premium d-flex justify-content-between align-items-center">
        <span><i class="fa-solid fa-user-graduate me-2"></i>Quản Lý Sinh Viên ({{ $sinhviens->total() }})</span>
        <div class="d-flex gap-2">
            ...
            <a href="{{ route('sinhvien.create') }}" class="btn btn-success btn-sm rounded-pill px-3"><i class="fa-solid fa-plus me-1"></i>Thêm SV Mới</a>
        </div>
    </div>
    <div class="card-body p-0">
        <table class="table table-custom table-hover mb-0">
            <thead><tr><th>Mã SV</th><th>Tài Khoản</th><th>Họ Tên</th><th>Lớp</th><th>Email</th><th>Trạng Thái</th><th class="text-center">Thao Tác</th></tr></thead>
            <tbody>
                @foreach($sinhviens as $item)
                <tr>
                    <td>{{ $item->MaSV }}</td>
                    <td><span class="badge bg-info text-dark">{{ $item->taiKhoan->TenDangNhap ?? '' }}</span></td>
                    ...
                    <td class="text-center">
                        ...
                        @endif
                        <a href="{{ route('sinhvien.edit', $item->MaSV) }}" class="btn btn-sm btn-light text-primary rounded-circle"><i class="fa-solid fa-pen"></i></a>
                        <form action="{{ route('sinhvien.destroy', $item->MaSV) }}" method="POST" class="d-inline" onsubmit="return confirm('Xóa SV này (sẽ xóa luôn tài khoản)?');">
                ...
                <div class="modal-body">
                    <div class="alert alert-info d-flex gap-2 align-items-start">
                        ...
                        <div>
                            ...
                            <code>TenDangNhap, HoTen, Email, SoDienThoai, MaLop</code><br>
                            Mật khẩu mặc định sau import: <strong>123456</strong><br>
                            <a href="{{ route('admin.import.template', 'sinhvien') }}" class="btn btn-sm btn-outline-success mt-2 rounded-pill px-3">
